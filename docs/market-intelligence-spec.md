...
The scheduled Celery entrypoint adds delivery-level idempotency: before calendar or provider I/O, a normal delivery reuses the newest already-published run for the same trading date and metric version. This makes broker redelivery stable even if Yahoo later revises a completed historical bar. An intentional provider correction must use the explicit internal `force_refresh=True` task option; that bypasses only the scheduled-delivery shortcut, then follows the normal content hash, audit, same-day revision, and atomic publication path. `PARTIAL` and `FAILED` dates are never short-circuited, so a later healthy delivery can recover them.
