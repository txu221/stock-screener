...
- [ ] Add `push.branches: [feat/market-intelligence-engine]` while retaining `workflow_dispatch`; limit paths to the workflow, `backend/**`, and Phase 2 report.
- [ ] Verify the YAML contains both triggers, commit `ci: run market intelligence integration on feature branch`, and push only to origin.
- [ ] Capture the real run ID/event/branch/SHA and inspect every job/log.
- [ ] For each failure, reproduce from logs, add the smallest regression assertion where production behavior changes, implement one minimal fix, and rerun.
- [ ] Require core PostgreSQL/Redis job green and optional Yahoo/Celery job green; record actual `SELECT version()` and `INFO server` Redis version in artifacts.
- [ ] Update `docs/phase2-integration-report.md` to `PHASE 2 COMPLETE` only after every Part A gate passes.
...
- [ ] Write failing domain tests for fixed pulse symbols SPY/QQQ/DIA/IWM, fixed ETF categories, leveraged/inverse exclusion, `MVP_METRIC_VERSION = "market_intelligence_mvp_v1"`, and derived-proxy wording.
- [ ] Implement immutable configuration/value objects with no provider calls.
- [ ] Write failing service tests using real SQLite tables for published-run selection, active S&P 500 membership, price > $5, RVOL20 excluding today, gains/losses/unusual-volume ordering, sector aggregation, ETF returns/RS/RVOL/drawdown, and insufficient history.
- [ ] Implement SQLAlchemy reads and pure calculations; missing values remain null and never become zero.
- [ ] Verify only existing database tables and dependencies are used.
...
- [ ] Write failing endpoint tests for overview pulse/freshness, top-20 gainers/losers/unusual volume, sector grouping, filter validation, ETF category ranking/score explanation, and empty/stale states.
- [ ] Add typed Pydantic response models with explicit `as_of`, `published_at`, `provider`, `metric_version`, and freshness.
- [ ] Add dependency wiring through the existing `get_db`; do not create a second FastAPI app.
- [ ] Extend the PostgreSQL API integration test to compare committed DB values with overview/movers/ETF responses.
- [ ] Run API and Phase 1 contract suites.
...
- [ ] Write failing API-client tests for exact URLs/query parameters and error propagation.
- [ ] Implement query helpers and stable query keys.
- [ ] Write failing navigation/route tests for the Market Intelligence primary entry and Today/Movers/Sectors/ETFs/Data Health subnavigation.
- [ ] Implement lazy routes and a compact MUI shell that follows the existing design tokens.
...
- [ ] Write failing loading/error/empty/stale/PARTIAL tests before components.
- [ ] Write failing tests proving SPY is benchmark-only, exactly 11 sectors render, 1D/5D/20D/60D switching uses server values, and rank directions include text/arrows.
- [ ] Implement accessible non-color-only cells and tooltips for Relative Strength, RVOL20, Flow Pressure, and Rank Change.
- [ ] Show As of, Last updated, Provider, metric version, and explicit stable snapshot date on every page.
...
- [ ] Write failing tests for server ordering, null display, top-20 limits, filter query changes, loading/error/empty states, and high-volume gain/loss distinction.
- [ ] Implement presentation-only tables; no return or RVOL calculation in JavaScript.
- [ ] Add sector grouping counts and freshness banner.
...
- [ ] Define and document score v1 as a deterministic percentile-weighted strength measure using 20D RS, 60D RS, 20D return, RVOL confirmation, and drawdown penalty; no expected-return language.
- [ ] Write failing backend score/rank/tie/version tests and frontend category/explanation/loading/error tests.
- [ ] Implement score components in Python and render them with an explanation panel.
- [ ] Verify leveraged/inverse ETFs are absent and category ranks are deterministic.
...
- [ ] Run Phase 1 exact, new backend unit/API, real PostgreSQL/Redis/Celery integration, full backend regression comparison, frontend focused/full tests, lint, and production build.
- [ ] Run manual Yahoo validation for SPY/XLK/XLE plus pulse/ETF symbols; reconcile AAPL/NVDA/MU movers when present without committing raw provider payloads.
- [ ] Run secret/device-state/dependency diff/security scans and record known baseline advisories without `audit fix`.
- [ ] Perform an independent review of financial semantics, stale data, proxy naming, ranking, score, S&P 500 scope, transaction boundaries, API schema, performance, and accessibility; fix findings test-first.
- [ ] Push final logical commits and require the latest integration workflow green.
- [ ] Create a draft PR to `txu221/stock-screener:main` only if every quality gate is green; never merge it.
- [ ] Record routes, screenshots when possible, run IDs, actual service versions, tests, limitations, and risks in the MVP report.
